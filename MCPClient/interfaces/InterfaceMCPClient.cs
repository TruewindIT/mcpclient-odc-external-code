...
namespace MCPClient.interfaces
{
    [OSInterface(
        Name = "MCPClient",
        Description = "Provides actions to interact with Model Context Protocol (MCP) servers using the Server-Sent Events (SSE) protocol. Allows OutSystems applications to call tools and retrieve information from connected MCP SSE endpoints.",
    ...
    public interface InterfaceMCPClient
    {
        ...
        string CallTool(
            [OSParameter(Description = "The SSE endpoint URL of the target MCP server (e.g., 'http://localhost:8888/sse').")] string ServerUrl,
